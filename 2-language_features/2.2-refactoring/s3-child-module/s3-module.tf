 ...
 * Fixed the logging bucket
 */
locals {
  logging_bucket = "porsche-compliance-log-${data.aws_caller_identity.current.id}-${var.region}"
}

resource "aws_s3_bucket_logging" "workshop_logging" {
  bucket        = aws_s3_bucket.workshop_s3_bucket.id
  target_bucket = local.logging_bucket # Writes logs in the same bucket it is not a good practice. :(
